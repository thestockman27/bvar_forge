# ADAPTED VERSION OF ORIGINAL bvar() 
...
bvar_mle <- function(
  ...
  ml_store <- ml_draw[["log_ml"]]
  hyper_store <- hyper_draw
  beta_store <- ml_draw[["beta_hat"]]
  sigma_store <- ml_draw[["sse"]] / ml_draw[["N"]]  #ml_draw[["omega_inv"]]
  ...
  if(verbose) {
    #close(pb)
    cat("Finished ML Calc after ", format(round(timer, 2)), ".\n", sep = "")
  }
